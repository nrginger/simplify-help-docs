...
# Константы
INPUT_FILE = "deepseek_2.csv"
OUTPUT_FILE = "deepseek_2_paraphrased.csv"
TEMP_FILE = "deepseek_2_paraphrased_temp.csv"
...
Важно: если предложение написано достаточно хорошо, оно короткое, без сложных оборотов и канцелярита, с прямым обращением к пользователю, не изменяй его.
...
def get_gpt_response(text, retry_count=3, use_full_prompt=True):
    """
    Получает ответ от DeepSeek API через прокси aitunnel.ru
    """
    ...
    for attempt in range(retry_count):
        try:
            client = openai.OpenAI(
                ...
                base_url="https://api.aitunnel.ru/v1/"  # Обратите внимание на слэш в конце
            )

            response = client.chat.completions.create(
                model="deepseek-chat-v3-0324",  # Используем модель из примера провайдера
                messages=messages,
                temperature=0.7,
                max_tokens=1000,  # Уменьшенное значение для безопасности
                top_p=0.9
            )
