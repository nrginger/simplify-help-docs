...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("paraphrase_claude.log"),
...
# Константы
INPUT_FILE = "claude_2.csv"  # Изменено на claude_1.csv
OUTPUT_FILE = "claude_2_paraphrased.csv"
TEMP_FILE = "claude_2_paraphrased_temp.csv"
...
PROMPT_FREQUENCY = 10  # Частота повторного включения инструкции (каждые 10 текстов)
...
def get_claude_response(text, retry_count=3, use_full_prompt=True):
    """
    Получает ответ от Claude 3.5 Haiku через прокси.
    ...
    """
    # Определение заголовков запроса внутри функции
    headers = {
    ...
    # Формируем запрос для Claude API
    payload = {
    ...
    for attempt in range(retry_count):
        try:
            response = requests.post(
                "https://caila.io/api/mlpgate/account/just-ai/model/claude/predict",
                headers=headers,
                json=payload,
                timeout=30
            )
            ...
            # Проверяем формат ответа и соответственно извлекаем текст
            if 'content' in response_data:
                # Стандартный формат Claude API
                return response_data['content'][0]['text']
            elif 'choices' in response_data and len(response_data['choices']) > 0:
                # Формат, похожий на GPT API
            ...
            logging.error(f"Попытка {attempt + 1}/{retry_count}: Ошибка при получении ответа от Claude: {e}")
            if attempt < retry_count - 1:
...
def main():
    ...
    for index, row in tqdm(df.iterrows(), total=len(df), desc="Перефразирование текстов"):
        ...
        paraphrased_text = get_claude_response(original_text, use_full_prompt=use_full_prompt)
